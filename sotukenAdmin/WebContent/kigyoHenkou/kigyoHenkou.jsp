...
<div id="level">
<ul id="level1">
	<li><a href="/sotukenAdmin/KigyoServlet?page=all">全表示</a></li>
</ul>
</div>
...
<form action="/sotukenAdmin/KigyoServlet" method="post">
	...
	<table id="table1" class="table01" rules="rows">
		...
		<c:forEach items="${km.list}" var="item">
			...
			<td style="width:30%"><a href="/sotukenAdmin/KigyoServlet?page=kigyoHensyu&id=${item.id }">${item.kigyoName }</a></td>
			<td style="width:30%"><a href="/sotukenAdmin/KigyoServlet?page=kigyoHensyu&id=${item.id }">${item.kigyoFuri }</a></td>
